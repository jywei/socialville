require "administrate/base_dashboard"

class UserDashboard < Administrate::BaseDashboard
  # ATTRIBUTE_TYPES
  # a hash that describes the type of each of the model's fields.
  #
  # Each different type represents an Administrate::Field object,
  # which determines how the attribute is displayed
  # on pages throughout the dashboard.
  ATTRIBUTE_TYPES = {
    statuses: Field::HasMany,
    messages: Field::HasMany,
    friendships: Field::HasMany,
    inverse_friendships: Field::HasMany.with_options(class_name: "Friendship"),
    id: Field::Number,
    username: Field::String,
    admin: Field::Boolean,
    email: Field::String,
    encrypted_password: Field::String,
    reset_password_token: Field::String,
    reset_password_sent_at: Field::DateTime,
    remember_created_at: Field::DateTime,
    sign_in_count: Field::Number,
    current_sign_in_at: Field::DateTime,
    last_sign_in_at: Field::DateTime,
    current_sign_in_ip: Field::String,
    last_sign_in_ip: Field::String,
    confirmation_token: Field::String,
    confirmed_at: Field::DateTime,
    confirmation_sent_at: Field::DateTime,
    unconfirmed_email: Field::String,
    created_at: Field::DateTime,
    updated_at: Field::DateTime,
    image_file_name: Field::String,
    image_content_type: Field::String,
    image_file_size: Field::Number,
    image_updated_at: Field::DateTime,
    bio: Field::Text,
    gender: Field::String,
    location: Field::String,
    age: Field::DateTime,
    cover_file_name: Field::String,
    cover_content_type: Field::String,
    cover_file_size: Field::Number,
    cover_updated_at: Field::DateTime,
    alt_email: Field::String,
    facebook: Field::String,
    facebook_permission: Field::Boolean,
    youtube: Field::String,
    youtube_permission: Field::Boolean,
    twitter: Field::String,
    twitter_permission: Field::Boolean,
    email_permission: Field::Boolean,
    birthday_permission: Field::Boolean,
  }.freeze

  # COLLECTION_ATTRIBUTES
  # an array of attributes that will be displayed on the model's index page.
  #
  # By default, it's limited to four items to reduce clutter on index pages.
  # Feel free to add, remove, or rearrange items.
  COLLECTION_ATTRIBUTES = [
    :statuses,
    :messages,
    :friendships,
    :inverse_friendships,
  ].freeze

  # SHOW_PAGE_ATTRIBUTES
  # an array of attributes that will be displayed on the model's show page.
  SHOW_PAGE_ATTRIBUTES = [
    :statuses,
    :messages,
    :friendships,
    :inverse_friendships,
    :id,
    :username,
    :admin,
    :email,
    :encrypted_password,
    :reset_password_token,
    :reset_password_sent_at,
    :remember_created_at,
    :sign_in_count,
    :current_sign_in_at,
    :last_sign_in_at,
    :current_sign_in_ip,
    :last_sign_in_ip,
    :confirmation_token,
    :confirmed_at,
    :confirmation_sent_at,
    :unconfirmed_email,
    :created_at,
    :updated_at,
    :image_file_name,
    :image_content_type,
    :image_file_size,
    :image_updated_at,
    :bio,
    :gender,
    :location,
    :age,
    :cover_file_name,
    :cover_content_type,
    :cover_file_size,
    :cover_updated_at,
    :alt_email,
    :facebook,
    :facebook_permission,
    :youtube,
    :youtube_permission,
    :twitter,
    :twitter_permission,
    :email_permission,
    :birthday_permission,
  ].freeze

  # FORM_ATTRIBUTES
  # an array of attributes that will be displayed
  # on the model's form (`new` and `edit`) pages.
  FORM_ATTRIBUTES = [
    :statuses,
    :messages,
    :friendships,
    :inverse_friendships,
    :username,
    :admin,
    :email,
    :encrypted_password,
    :reset_password_token,
    :reset_password_sent_at,
    :remember_created_at,
    :sign_in_count,
    :current_sign_in_at,
    :last_sign_in_at,
    :current_sign_in_ip,
    :last_sign_in_ip,
    :confirmation_token,
    :confirmed_at,
    :confirmation_sent_at,
    :unconfirmed_email,
    :image_file_name,
    :image_content_type,
    :image_file_size,
    :image_updated_at,
    :bio,
    :gender,
    :location,
    :age,
    :cover_file_name,
    :cover_content_type,
    :cover_file_size,
    :cover_updated_at,
    :alt_email,
    :facebook,
    :facebook_permission,
    :youtube,
    :youtube_permission,
    :twitter,
    :twitter_permission,
    :email_permission,
    :birthday_permission,
  ].freeze

  # Overwrite this method to customize how users are displayed
  # across all pages of the admin dashboard.
  #
  # def display_resource(user)
  #   "User ##{user.id}"
  # end
end
